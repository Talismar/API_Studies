...
function TableListSystem({seller_id}) {
  ...
  const newListSystem = async () => {
    if (nameSeller.current.value) {
      await api_action('POST', 'SystemList/', "system", {'name': nameSeller.current.value, "seller": seller_id})
    }
  ...
  return (
      ...
      <div className="modal-dialog">
        <div className="modal-content">
          ...
          <div className="modal-body">
            ...
            <label>Nome Da Nova Lista:</label><br />
            <input type="text" ref={nameSeller}/><br />
            
          </div>
          <div className="modal-footer">
            <button type="button" className="btn btn-secondary" data-bs-dismiss="modal">Close</button>
            <button type="button" className="btn btn-primary" onClick={newListSystem}>Criar Nova Lista</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className='d-grid gap-2 d-md-flex justify-content-md-end mt-4'>
      ...
      <button type="button" className="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">Nova Lista</button>
    </div>
          ...
          systemList.listsystem_set.map((values, keys) => {
            return (
              <tr key={keys}>
                ...
                <td>{values.name}</td>
                <td>
                  <Link to={`/listPurchase/${values.id}`}><FontAwesomeIcon icon={faPenToSquare}/></Link>
                  &nbsp;&nbsp;
