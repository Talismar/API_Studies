...
function TableListSeller({ sellerList, api_action }) {
  ...
  const newSeller = async () => {
    if (nameSeller.current.value) {
      await api_action('POST', 'SellerList/', "seller", {'name': nameSeller.current.value})
    }
  ...
  return (
      ...
      <div className="modal-dialog">
        <div className="modal-content">
          ...
          <div className="modal-footer">
            ...
            <button type="button" className="btn btn-primary" onClick={newSeller}>Cadastrar Novo Vendedor</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div className='d-grid gap-2 d-md-flex justify-content-md-end mt-4'>
      <button type="button" className="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">Novo Vendedor</button>
    </div>
    ...
    <tbody>
        ...
        {
          sellerList.length > 0 && sellerList.map((values, keys) => {
            return (
              <tr key={keys}>
